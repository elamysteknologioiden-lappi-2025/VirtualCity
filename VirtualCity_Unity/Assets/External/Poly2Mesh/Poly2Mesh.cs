...
//			Debug.Log("Rotated " + originalPos.ToString("F4") + " to " + p.ToString("F4"));
    ...
    public static GameObject CreateGameObject(Polygon polygon, GameObject aGob) {
        aGob.AddComponent(typeof(MeshRenderer));
        MeshFilter filter = aGob.AddComponent(typeof(MeshFilter)) as MeshFilter;
        filter.mesh = CreateMesh(polygon);
        return aGob;
    }
}
